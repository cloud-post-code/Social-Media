...
const AssetGenerator: React.FC<AssetGeneratorProps> = ({ activeBrand, onAssetCreated, initialAsset, onExitEditing }) => {
  ...
  // Generate image from frontend preview using html2canvas (exact CSS match)
  const generateImageFromPreview = async (): Promise<string> => {
    if (!imageContainerRef.current || !imageDimensions) {
      throw new Error('Image container not ready');
    }
  ...
  // Manage contentEditable via refs to avoid React DOM reconciliation issues
  // This prevents React from recreating DOM nodes when toggling edit mode
  useEffect(() => {
    if (titleTextRef.current) {
      const shouldBeEditable = editingTextElement === 'title';
      const currentValue = titleTextRef.current.contentEditable;
      const newValue = shouldBeEditable ? 'true' : 'false';
      ...
      if (currentValue !== newValue) {
        ...
        requestAnimationFrame(() => {
          if (titleTextRef.current) {
            titleTextRef.current.contentEditable = newValue;
          }
        });
      }
    }
  ...
  useEffect(() => {
    if (subtitleTextRef.current) {
      const shouldBeEditable = editingTextElement === 'subtitle';
      const currentValue = subtitleTextRef.current.contentEditable;
      const newValue = shouldBeEditable ? 'true' : 'false';
      ...
      if (currentValue !== newValue) {
        ...
        requestAnimationFrame(() => {
          if (subtitleTextRef.current) {
            subtitleTextRef.current.contentEditable = newValue;
          }
        });
      }
    }
  ...
  return (
    <div className="space-y-12 max-w-6xl mx-auto pb-20 px-4 sm:px-0">
      {/* Back/Exit Button when editing an asset */}
      {displayAsset && onExitEditing && (
        <div className="flex items-center justify-between">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 text-slate-600 hover:text-slate-900 font-bold text-sm transition-colors rounded-xl hover:bg-slate-100"
          >
